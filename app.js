...
app.use('/demo/assignments/:id', (req, res, next) => {
  if (req.method === 'PATCH') {
...
app.use((req, res, next) => {
  if (req.originalUrl !== '/healthz' && req.originalUrl !== '/demo/assignments') {
    if (
      req.originalUrl.startsWith('/demo/assignments/') &&
      req.originalUrl.endsWith('/submission') &&
      req.method !== 'POST'
    ){
      // Allow only POST requests for /demo/assignments/:id/submission
      logger.warn('Invalid method for /demo/assignments/:id/submission');
      res.status(405).json();
    }else{
    ...
    (req.originalUrl == '/demo/assignments' && 
